...
- `code-locale` — the domain model past the zod parser is English — that parser boundary is the anti-corruption layer even when the repo's prose is not; a domain term with no faithful translation is kept only when the item's glossary lists it.
