...
8. **Tests** — add/update tests per the issue's test strategy and the repo's framework. A test suite
   ...
   purpose afterwards is `bug-hunter`, a different job. Where the repository or the user runs the
   red-green cycle, this step moves ahead of step 7 and its doctrine is `tdd`; the order above stays
   the default until they say so.
