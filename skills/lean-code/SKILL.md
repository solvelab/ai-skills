...
smallest thing that fails if the logic breaks: one small `test_*.py` beside the
module, in the project's test home when it has one. An inline `assert`-based `__main__` self-check
is for a single-file script that nothing imports — a module keeps its check outside itself, so the
product file stays the minimum. No frameworks, no fixtures, no per-function suites unless asked. Trivial one-liners need no
...
## What the catalog measured

Measured on this catalog's own harness (linked below), on what the maintainer runs every day:
`claude-opus-5[1m]` (`--model 'opus[1m]'`), Claude Code `2.1.261`, 9 tasks, `n=3` per task (`n=5`
on `cache` and `csv-sum` after the protocol's dispersion clause), 2026-09-05/06, one machine;
`added_lines` = the `+N` of production files by `git diff`, tests excluded.

- **Baseline** (the maintainer's real rules, no skill, 27 cells): `correct` 27/27, `safe` 27/27 —
  no guard dropped, nothing re-implemented, no caller-only patch. The over-build was concentrated:
  a custom exception class for a guard 6/27, speculative input tolerance 6/27, helper decomposition
  of a short loop 4/27, type checks nobody asked for 3/27, docstring expansion 12/27, `no_check`
  3/27 (the structural task), prose longer than the diff 1/27.
- **Skill arm** (baseline + the always-on *Lean Code* block + this skill as a project skill, 31
  cells): `correct` 31/31, `safe` 31/31; mean `added_lines` over the pre-registered over-build
  group **−34.4 %** against the baseline — `safe-path` 67.7 → 13.7 (min–max 12–16), `cache`
  12.7 → 5.4 (3–11), `fastapi-create-item` 16 → 16.7 (16–18), `react-use-orders` 22 → 21 (21–21);
  worst task +4.2 %; the `skipped: …, add when …` trailer in 27/31 cells; the two-caller bug fixed
  at its root 3/3; new dependency 0/31. Verdict by the letter of the protocol: **SHIP**, after one
  REWRITE of this skill's one-check rule (`reuse-slug` +37.0 % → −3.7 %) and the +2 repetitions
  the dispersion clause asked for on three tasks.

The group Δ is the only number this skill publishes; every per-task mean, its min–max, the block
arm alone (−28.6 %, INCONCLUSIVE), the spend and what the measurement does not cover (one model
id, one CLI version, one machine, two days, small counts with wide spreads) are in
...
- `agent-delegation` — the "which kind of thing" counterpart of this skill's "how much": once
  something must exist, it decides where the rule lives (a CI script, a hook or a skill) and, apart
  from that, whether one piece of work runs in the calling loop or in a subagent. Rung 1 here — does
  this need to exist at all — outranks both of its questions.
