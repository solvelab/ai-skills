...
- `code-locale` — plugin config keys, YAML file names and chat-command names are English even when the repo's prose is not; a domain term with no faithful translation is kept only when the item's glossary lists it.
