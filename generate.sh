...
declare -A GROUP_THEME=(
  ...
  [game]="React Three Fiber conventions for 3D on the web"
  [assettoserver]="AssettoServer dedicated-server operation, C# plugins and CSP Lua online scripts"
  [devops]="Kubernetes/Helm migration and cluster resource tuning"
  [docs]="Project documentation against a fixed document map"
  [workflow]="Commit format, OpenSpec spec-driven workflow, the backlog rite, the anti-guessing rite, the code-locale rule, the lean-code doctrine and the artifact-boundary rule for delegation"
