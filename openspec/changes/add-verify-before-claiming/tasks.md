...
- [ ] 1.1 Create `skills/verify-before-claiming/SKILL.md` with the 5 contracted frontmatter keys,
      `metadata.category: process`, and a description carrying pt-BR + English triggers plus four
      `Do NOT use for` boundaries
- [ ] 1.2 Body: the claim definition (asserted **or acted on**), `## CRITICAL: Ground rules` in the
      numbered `**Bold lead** — explanation` shape used by `backlog/SKILL.md:36-51`
- [ ] 1.3 Body: the research ladder (rungs 0-6), its stop condition, the never-skip-downward rule,
      the rung-4/5-depends-on-rung-2 rule, and offline degradation
- [ ] 1.4 Body: the cost rule (when the ladder does **not** run) and the anti-theater clause
- [ ] 1.5 Body: claim labelling (verified / inferred / unknown), the load-bearing threshold, and the
      in-code `UNVERIFIED:` comment rule
- [ ] 1.6 Body: the not-found protocol and the ban on substituting a plausible answer
- [ ] 1.7 Body: the knowledge-cutoff rule and the off-script guard, the latter linking to
      `execute-backlog` for deviation bookkeeping instead of restating it
- [ ] 1.8 `references/research-ladder.md` — per-ecosystem lockfile and installed-source locations,
      probe recipes, version-pinned documentation URLs, offline degradation table
- [ ] 1.9 `references/report-templates.md` — the three labels, the not-found report, the
      Doing/Not-doing/Assumptions block, the mid-flight scope-change report
- [ ] 1.10 `references/failure-catalog.md` — anti-pattern table where every row names the defect that
      earned it, plus the add/remove rule
- [ ] 1.11 Probe every command the skill prescribes on this machine and record the tool versions and
      the date in the skill body

## 2. Cross-links and canonical map

- [ ] 2.1 `skills/documentation/SKILL.md` — shrink lines 34-36 to one line + link; add a `See also`
      bullet
- [ ] 2.2 One-line link in `bug-hunter`, `api-resilience-testing`, `backlog`, `execute-backlog`,
      `assettoserver-csp-lua`, `assettoserver-plugin`, `helm-migration`, `openspec-drivezone`
- [ ] 2.3 Bump `metadata.version` on every touched skill

## 3. Catalog composition

- [ ] 3.1 `README.md` — new row in the Process & git table
- [ ] 3.2 `generate.sh` `GROUP_DESC[workflow]` and `.claude-plugin/marketplace.json`
      `ai-skills-workflow` description name the new skill
- [ ] 3.3 Run `./generate.sh` and commit the regenerated `claude/ codex/ cursor/ copilot/ plugins/`
...
- [ ] Q.5 `python3 scripts/validate-skills.py` exits 0 (C1 reference paths, C2 cross-skill refs,
      C3 code blocks parse, C4 description vs body, C6 fence tags, C8 no meta sections)
- [ ] Q.6 `python3 scripts/selftest-validate-skills.py` and `python3 scripts/scan-secrets.py` pass
- [ ] Q.7 No `## How to Use` / `## Usage` / `## Prompt` / `## Trigger Test Cases` section anywhere in
...
- [ ] V.1 `openspec validate add-verify-before-claiming --strict` green
- [ ] V.2 Catalog discovery intact: every skill directory carries a `SKILL.md` whose `name` matches
