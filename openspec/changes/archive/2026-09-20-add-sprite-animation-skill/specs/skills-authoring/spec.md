...
what never is, when the register yields and where it does not apply — → `terse-response`; reproducing ready-made frame-sheet art — reading the sheet, normalising scale between pose groups, the single cell, the frame rate and the computation that keeps the feet from sliding — → `sprite-animation`.
