...
- Use **explicit file order** in `client_scripts`/`server_scripts`/`shared_scripts`. Avoid glob
  `**/*.lua` — load order is undefined and breaks module-extends-global patterns.
...
- New Lua files require a full restart (hot-reload only re-reads files already in the manifest cache).

## See also

- `fivem-fallback` — resilience when calling an external backend from Lua.
- `bug-hunter` — adversarial testing, including the FiveM/Lua track (event injection, StateBag races).
