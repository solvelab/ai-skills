...
creature({
  ...
  build(host) {
    ...
    const bellyBand = body.map((s) => ({ x: s.x, back: -s.belly * 0.30, belly: s.belly * 0.92 }))
    ...
    // Dorsal fin: falcate — swept back with a CONCAVE trailing edge. Its leading edge leaves the
    // back at ~42% and the tip falls behind the base, which is what distinguishes it from the
    // upright triangle of a shark.
    el(root, 'path', {
      d: `M${(0.5 - 0.36) * L} ${-0.106 * L} `
       + `C${(0.5 - 0.40) * L} ${-0.17 * L}, ${(0.5 - 0.45) * L} ${-0.20 * L}, ${(0.5 - 0.50) * L} ${-0.205 * L} `
       + `C${(0.5 - 0.465) * L} ${-0.165 * L}, ${(0.5 - 0.455) * L} ${-0.125 * L}, ${(0.5 - 0.455) * L} ${-0.098 * L} Z`,
      fill: skin,
    })
...
creature({
  ...
  build(host) {
    ...
    // Dorsal: small, far back, sitting on the hump.
    el(root, 'path', {
      d: `M${(0.5 - 0.63) * L} ${-0.104 * L} `
       + `C${(0.5 - 0.655) * L} ${-0.145 * L}, ${(0.5 - 0.685) * L} ${-0.162 * L}, ${(0.5 - 0.715) * L} ${-0.160 * L} `
       + `C${(0.5 - 0.70) * L} ${-0.138 * L}, ${(0.5 - 0.695) * L} ${-0.115 * L}, ${(0.5 - 0.70) * L} ${-0.095 * L} Z`,
      fill: skin,
    })
