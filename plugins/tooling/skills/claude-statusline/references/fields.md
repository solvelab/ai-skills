...
| — | **There is no cumulative token field.** `cost.total_cost_usd` is the only running total the payload carries. A session token total has to be accumulated by the script itself: key the state file on `session_id`, and commit a turn only when `prompt_id` changes — the status line re-renders many times per turn, so a naive `+=` multiplies. See `statusline.sh`. **Never split that state — or the payload — on `\t`**: tab is an IFS *whitespace* character, so bash collapses runs of it and drops empty fields, shifting every later field left. One absent value silently corrupts the accumulator. Use `\x1f`. **And do not key the accumulation on `prompt_id`**: `current_usage` is the usage of the *last API call*, not of the turn, and one turn makes many calls — captured live, the same `prompt_id` produced `out=627 -> 480 -> 587`, non-monotonic. Bank when the usage tuple changes; identical consecutive renders then bank nothing. |
