...
| `cost.total_api_duration_ms` | Time waiting on the API. Over `total_duration_ms` this is the share of the session spent on model latency rather than on everything else — 55% in an observed payload (1 840 177 of 3 348 802 ms). `statusline.sh` renders it as the `🌐 api` meter. Optional: guard the division, a status line that divides by zero is a broken render. |
