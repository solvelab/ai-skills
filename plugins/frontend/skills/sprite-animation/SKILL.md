...
> **Not version-bound, with three named exceptions.**
> This skill does not depend on a tool version: the CSS rules are specification-level (how
> `background-position` resolves a percentage, how `steps()` divides an animation) and the
> geometry rules are properties of the art, not of any renderer. Three surfaces
> in `references/` are **not** version-free and are not covered by that claim: the Pillow/NumPy
> cutting recipe, the `playwright-core` call `emulateMedia({ reducedMotion: 'no-preference' })`,
> and the assertion that headless Chromium reports `reduce` by default. The last was observed on
> the Chromium of the Playwright cache on 2026-09-19 and again on 2026-09-20; the first two were
> exercised on those same runs and their exact versions were **not** recorded, so treat them as
> probed-once, unpinned. The pixel numbers quoted as evidence come from one measured scene
...
| Scale matched by a single feature | Hair width said 1.60, which implies a seated figure as tall as a standing one | [No proxy across views](#scale-is-not-measured-by-proxy) |
...
against a front view produces a number that is precise and meaningless.

What caught it was not a better measurement, it was a falsification. Hair width put the seated
group at **1.60** of the walking group; dividing the seated figures by 1.60 left them 182 px tall
against 184 px for the standing pose — a seated person the same height as a standing one, which
cannot be. A second attempt by face width was abandoned before producing a number: the skin mask
that worked on one group found nothing on the other, which is the same defect wearing a different
hat.
...
- `stride` — **foot-centre to foot-centre** at maximum extension, divided by the figure height,
  taken as the maximum over the frames of the cycle and read from the alpha channel. Group the
  opaque columns in the bottom tenth of the frame into two clusters and take the distance between
  their centres.

  The plausible wrong one is the bounding-box extremes — leading toe to trailing heel — which
  includes one whole shoe. On the measured art it gave **230 px** against the correct **148 px**:
  55% too large, which makes the derived duration 55% too long and the feet slide the other way.
...
That example runs at 1.283 heights per second, well above the 0.85 the same art gives at the
conventional 10 fps, and the scene's author chose it deliberately: at 10 fps the figure needs
3.97 s to cross that room, which is slow for something on screen all day. The rule the example
obeys is the one this section is about — rate and travel agree, so nothing slides — and the
speed within that is a pace decision. What the example must not do is disagree with itself, and
it does not.
...
Sanity check on the result, and a rule of thumb with no source behind it: a person walking at a
comfortable pace covers roughly 0.8 figure heights per second. It is here to catch an order of
magnitude, not to be derived from. The measured art at 10 fps gives 0.855 — so an 8-frame cycle at the
...
The routing away from this skill is in its description, not repeated here. What the body carries
is the two canonical homes it borrows from: **`verify-before-claiming`** for claiming a number
without measuring it, and **`lean-code`** for the `lean:` ceiling marker used above.
