...
**When the art already exists as numbered frames**, none of this applies: the object does not have
to be understood, it has to be replayed without being distorted. That is the `sprite-animation`
skill, and it owns reading the sheet, matching scale between pose groups, the shared cell and the
frame rate.
