...
- Subject line language follows the repo's existing convention (solvelab project repos use
  Portuguese; this catalog repo uses English) — check `git log` and match it. The **code** the
  commit carries is English regardless, and so is the `scope`, which names a module (`code-locale`).
